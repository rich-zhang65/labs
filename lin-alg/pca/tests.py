...
def test_matrix(target_P, target_X0, target_X1):
    ...
    for test_case in test_cases:
        ...
        try:
            assert np.allclose(np.diagonal(target_P),
            ...
            failed_cases.append(
                {
                    "name": test_case["name"],
                    "expected": np.nonzero(
                        np.logical_not(np.isclose(np.diagonal(
                            target_P), np.diagonal(test_case["expected"]["P"])))
                    ),
                    "got": sum(target_P),
                }
            )
            ...
            failed_cases.append(
                {
                    ...
                    "expected": np.nonzero(
                        np.logical_not(np.isclose(
                            sum(target_P), sum(test_case["expected"]["P"])))
                    ),
                    "got": sum(target_P),
                }
            )
